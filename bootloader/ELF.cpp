...
{
    // Copy phdrs
    prog_hdrs = (Elf32_Phdr*)malloc(sizeof(Elf32_Phdr) * elf32Ehdr->e_phnum);
    for (int k = 0; k < this->global_hdr.e_phnum; ++k)
        prog_hdrs[k] = *(Elf32_Phdr*)((uint)elf32Phdr + this->global_hdr.e_phentsize * k);

    // Copy shdrs
    section_hdrs = (Elf32_Shdr*)malloc(sizeof(Elf32_Shdr) * elf32Ehdr->e_shnum);
    ...
    for (int k = 0; k < this->global_hdr.e_shnum; ++k)
    {
        ...
        dyn_table = (Elf32_Dyn*)malloc(sizeof(Elf32_Dyn) * num_entries);
    ...
    for (int k = 0; k < this->global_hdr.e_phnum; ++k)
    {
        ...
        interpreter_name = (char*)malloc(sizeof(char) * (strlen(interpr_path) + 1));
    ...
    shstrtab = (char*)malloc(sizeof(char)*sh_strtab_h->sh_size);
    ...
    for (int k = 0; k < elf32Ehdr->e_shnum; ++k)
    {
        ...
        plt_relocs = (Elf32_Rel*)malloc(sizeof(Elf32_Rel) * num_entries);
    ...
    for (int k = 0; k < elf32Ehdr->e_shnum; ++k)
    {
        ...
        dyn_relocs = (Elf32_Rel*)malloc(sizeof(Elf32_Rel) * num_entries);
        for (uint i = 0; i < num_entries; i++)
            dyn_relocs[i] = *r++;
        break;
    }
    ...
    for (int k = 0; k < elf32Ehdr->e_shnum; ++k)
    {
        ...
        if (h->sh_type != SHT_DYNSYM)
            continue;
        dynsym_hdr = (Elf32_Shdr*)malloc(sizeof(Elf32_Shdr));
        *dynsym_hdr = *h;
        Elf32_Shdr* dyn_strtab_h = &section_hdrs[h->sh_link];
        dynsym_strtab = (char*)malloc(sizeof(char*) * dyn_strtab_h->sh_size);
    ...
    if (lib_name_idx != (uint)-1)
    {
        ...
        auto cpy = (char*)malloc(sizeof(char) * (strlen(lib_name) + 1));
        strcpy(cpy, lib_name);
        lib_name = cpy;
    }
}
