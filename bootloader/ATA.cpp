...
void IDE::init()
{
    ...
    for (int i = 0; i < 2; i++)
        for (j = 0; j < 2; j++)
        {
            ...
            sleep_1ms(); // Wait 1ms for drive select to work.

            // (II) Send ATA Identify Command:
            write(i, ATA_REG_COMMAND, ATA_CMD_IDENTIFY);
            sleep_1ms(); // This function should be implemented in your OS. which waits for 1 ms.
...
void IDE::read_buffer(unsigned char channel, unsigned char reg, void* buffer, uint quads)
{
     ...
     *           assembly blocks.
     *
     * The issue has been solved by moving asm instructions to IO::insl.
     */
    if (reg > 0x07 && reg < 0x0C)
        write(channel, ATA_REG_CONTROL, 0x80 | channels[channel].nIEN);
...
unsigned char IDE::polling(unsigned char channel, uint advanced_check)
{
    ...
    // -------------------------------------------------
    while (read(channel, ATA_REG_STATUS) & ATA_SR_BSY); // Wait for BSY to be zero.
...
unsigned char IDE::print_error([[maybe_unused]] uint drive, unsigned char err)
{
    if (err == 0)
        return err;
...
__attribute__((optimize("O0"))) // I did not find any other way to prevent optimization conflicts with asm instructions
                          ...
                          unsigned short selector, uint edi)
{
    ...
    else if (IDE::devices[drive].Capabilities & 0x200)
    {
        // Drive supports LBA?
        // LBA28:
        lba_mode = 1;
    ...
    else
    {
        ...
        IDE::write(channel, ATA_REG_COMMAND, (unsigned char []){
                       ATA_CMD_CACHE_FLUSH,
                       ATA_CMD_CACHE_FLUSH,
