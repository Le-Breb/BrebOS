...
/*
// xHci Spec Section 6.4.2.1 Figure 6-15: Transfer Event TRB

Generated by the xHC on the Event Ring whenever a Transfer TRB (Setup/Data/
Status Stage or Normal) completes, or an error occurs while processing one.
TRB Pointer references the TRB that generated the event; TRB Transfer Length
holds the number of bytes *not* transferred (e.g. non-zero on a short packet).
*/
typedef struct xhci_transfer_event_request_block {
    ...
    struct {
        ...
        uint32_t rsvd1       : 7;
        uint32_t trb_type    : 6;
        uint32_t endpoint_id : 5; // Device Context Index (DCI) of the endpoint
        uint32_t rsvd2       : 3;
        uint32_t slot_id     : 8;
    };
} xhci_transfer_event_trb_t;
static_assert(sizeof(xhci_transfer_event_trb_t) == sizeof(uint32_t) * 4);
