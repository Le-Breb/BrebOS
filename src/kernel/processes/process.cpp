...
ELF* Process::load_lib(const char* path, void* lib_dynlk_runtime_entry_point)
{
    ...
    {
        uint sys_pe_id = pte[(lib_runtime_load_addr + got_runtime_addr) / PAGE_SIZE];
        uint sys_pde_id = sys_pe_id / PDT_ENTRIES;
        uint sys_pte_id = sys_pe_id % PT_ENTRIES;
...
Process* Process::allocate_proc_for_elf_module(ELF* elf)
{
...
size_t Process::write_args_to_stack(size_t stack_top_v_addr, int argc, const char** argv, list<Elf32_Addr>
...
Process* Process::from_memory(uint start_addr, pid_t pid, pid_t ppid, int argc, const char** argv)
{
    ...
    proc->stack_state.esp = write_args_to_stack(p_stack_top_v_addr, argc, argv, proc->init_fini.init_array,
                                                proc->init_fini.fini_array);
...
void
Process::copy_elf_subsegment_to_address_space(void* bytes_ptr, uint n, Elf32_Phdr* h, uint& pte_offset,
...
void Process::map_elf(ELF* load_elf, uint pte_offset)
{
    uint* elf_pte = pte + pte_offset;
