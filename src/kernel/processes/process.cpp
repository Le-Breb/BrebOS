...
ELF* Process::load_lib(const char* path, void* lib_dynlk_runtime_entry_point)
{
	...
	ELF* lib_elf;
	if (!((lib_elf = ELF::is_valid(start_address, SharedObject))))
		return nullptr;
	...
	{
		uint sys_pe_id = pte[(lib_load_addr + got_runtime_addr) / PAGE_SIZE];
		uint sys_pde_id = sys_pe_id / PDT_ENTRIES;
		uint sys_pte_id = sys_pe_id % PT_ENTRIES;
...
uint Process::load_elf(ELF* load_elf, uint elf_start_address)
{
