...
{
    Process* proc;
    if (!((proc = load_process(path, p->pid, p->ppid, argc, argv, envp))))
        return false;
...
int Scheduler::exec(const char* path, pid_t ppid, int argc, const char** argv, const char** envp)
{
    ...
    Process* proc;
    if (!((proc = load_process(path, pid, ppid, argc, argv, envp))))
        return -1;
...
Process* Scheduler::load_process(const char* path, pid_t pid, pid_t ppid, int argc, const char** argv, const char** envp)
{
    if (ready_queue->full())
    {
        printf_error("Max process are already running");
...
void Scheduler::wake_up_process_parent(pid_t process_pid)
{
    ...
    if (auto wstatus_addr = (int*)waiting_process->cpu_state.ecx)
...
void Scheduler::start_kernel_process(void* eip)
{
    ...
    // Create process
    auto p = new Process(nullptr, 0, Memory::page_tables, Memory::pdt, &stack_state,
