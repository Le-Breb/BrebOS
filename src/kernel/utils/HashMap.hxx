...
template <typename Key, typename T, uint32_t capacity, typename hash_func, typename equal_func>
template<typename K>
requires std::invocable<hash_func, const K&>
size_t HashMap<Key, T, capacity, hash_func, equal_func>::bucket(const K& key) const
{
...
size_t HashMap<Key, T, capacity, hash_func, equal_func>::get_index(const K& element) const
{
...
template <typename Key, typename T, uint32_t capacity, typename hash_func, typename equal_func>
void HashMap<Key, T, capacity, hash_func, equal_func>::advance_index(uint32_t& index)
{
    index = (index + 1) & (capacity - 1);
}

template <typename Key, typename T, uint32_t capacity, typename hash_func, typename equal_func>
HashMap<Key, T, capacity, hash_func, equal_func>::~HashMap()
{
    for (auto& e : elements)
        if (e.used)
...
template <typename Key, typename T, uint32_t capacity, typename hash_func, typename equal_func>
Status HashMap<Key, T, capacity, hash_func, equal_func>::add(const T& element)
{
    if (size == capacity)
...
template <typename Key, typename T, uint32_t capacity, typename hash_func, typename equal_func>
bool HashMap<Key, T, capacity, hash_func, equal_func>::is_present(const T& t) const
{
    return get_index(t) != capacity;
}

template <typename Key, typename T, uint32_t capacity, typename hash_func, typename equal_func>
void HashMap<Key, T, capacity, hash_func, equal_func>::remove(const T& element)
{
...
template <typename K>
requires kdetail::FindCompatible<hash_func, equal_func, T, K>
Optional<const T*> HashMap<Key, T, capacity, hash_func, equal_func>::find(const K& element) const
{
    uint32_t index = bucket(element);
    while (elements[index].used)
    {
...
template <typename Key, typename T, uint32_t capacity, typename hash_func, typename equal_func>
[[nodiscard]]
uint32_t HashMap<Key, T, capacity, hash_func, equal_func>::get_size() const
{
    return size;
}

template <typename Key, typename T, uint32_t capacity, typename hash_func, typename equal_func>
bool HashMap<Key, T, capacity, hash_func, equal_func>::is_full() const
{
    return size == capacity;
}

template <typename Key, typename T, uint32_t capacity, typename hash_func, typename equal_func>
typename HashMap<Key, T, capacity, hash_func, equal_func>::Iterator HashMap<Key, T, capacity, hash_func, equal_func>::
begin()
{
