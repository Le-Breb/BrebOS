...
It is designed to work under QEMU, but it has also successfully been tested on a Lenovo IdeaPad 320!<br>
...
All programs are placed by the build system in `/bin` in the OS disk image. Under the hood, programs come from two places:
- `BrebOS/src/programs`: programs written by me, as well as your own programs (see next section).
- busybox: programs listed in `BrebOS/src/busybox_config/programs.list` directly come from busybox.

> Getting BusyBox programs running on BrebOS is one of my proudest achievements - it's a real stress test of everything the OS can do.

Here is the list of the main programs present:
...
- **File System 📁**
    ...
    - USB driver (half taken from [a series of youtube tutorials](https://www.youtube.com/watch?v=W0EqQiDSoxE&list=PLATP7rOKo3E82tBnMp90B4zejpWeAKlxn), half generated by Claude)
      - USB
      - BOT
      - SCSI
      - xHCI
    - SuperFloppy and MBR support
    - FAT32 support
